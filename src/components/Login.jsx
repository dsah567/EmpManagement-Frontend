  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (err.response && err.response.data) {
            setError(err.response.data.message || "Invalid authorization");
        } else {
    ...
    <div className={`flex justify-center items-center min-h-screen ${error ? 'bg-red-50' : 'bg-white'}`}>
            <div className="w-full max-w-md p-8 space-y-4 rounded-lg shadow-lg bg-gray-100 dark:bg-gray-900">
                ...
                <p className="text-center text-gray-600 dark:text-gray-400">
                    Don't have an account? <a href="/signup" className="text-indigo-600 hover:underline dark:text-indigo-400">Sign Up</a>
                </p>
            </div>
        </div>
  )
